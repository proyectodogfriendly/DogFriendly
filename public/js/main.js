  ...
  window.onscroll = () => {
    ...
    isInViewport(document.querySelector(".hero-inertia"))
      ? (document.querySelector(
          ".hero-inertia"
        ).style.transform = `translateY(${winScroll * 0.3}px)`)
      : null;

    // Movile devices horizontal transitions
    if (isInViewport(document.querySelector(".features"))) {
      mobiles[0].style.transform = `translateX(${mobiles[0].getBoundingClientRect()
        .top * 0.3}px)`;
      mobiles[1].style.transform = `translateX(${mobiles[1].getBoundingClientRect()
        .top * 0.2}px)`;
    }

    // Message box transitions
    // if (isInViewport(document.querySelector(".facts"))) {
    //   messages[0].style.transform = `translate(${-messages[0].getBoundingClientRect()
    //     .top * 0.2}px, ${messages[0].getBoundingClientRect().top * 0.1}px)`;
    //   messages[1].style.transform = `translate(${messages[1].getBoundingClientRect()
    //     .top * 0.1}px, ${messages[1].getBoundingClientRect().top * 0.05}px)`;
    //   messages[2].style.transform = `translate(${messages[2].getBoundingClientRect()
    //     .top * 0.3}px, ${-messages[2].getBoundingClientRect().top * 0.05}px)`;
    // }

    // Slider transition
    // if (isInViewport(document.querySelector(".join"))) {
    //   sliderTrack.style.transform = `translateX(${-document
    //     .querySelector(".join")
    //     .getBoundingClientRect().top * 0.6}px)`;
    // }
...
//     mobilescreens[0].style.transform = `translate(${e.x * 0.03}px, ${-e.y *
//       0.03}px)`;
//     mobilescreens[1].style.transform = `translate(${e.x * 0.06}px, ${-e.y *
//       0.06}px)`;
//     mobilescreens[2].style.transform = `translate(${e.x * 0.09}px, ${-e.y *
//       0.09}px)`;
//     mobilescreens[3].style.transform = `translate(${e.x * 0.12}px, ${-e.y *
//       0.12}px)`;
//   };
